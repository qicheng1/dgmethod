...
int main(int argc, char * argv[])
{
  ...
  StiffMatrix2 stiff_matrix(dgfem_space);
  stiff_matrix.algebricAccuracy() = 10;
  stiff_matrix.build();

  std::ofstream output("matr.txt");
  stiff_matrix.print(output);
  output.close();

  FEMFunction<double,2> solution(dgfem_space);
  Vector<double> right_hand_side;
  Operator::L2Discretize(&f, dgfem_space, right_hand_side, 10);
  L2DGDiscretize(&g, dgfem_space, right_hand_side,10);

  std::ofstream output2("vect.txt");
